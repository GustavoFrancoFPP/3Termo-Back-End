                    ...
                    <div class="flex flex-col justify-between border border-gray-200 p-5 rounded-lg hover:shadow-lg transition bg-gray-50">
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mb-1">
                                <span class="font-semibold text-gray-800">Cliente:</span> {{ $pedido->cliente->nome ?? 'N/A' }}
                            </p>
                            <p class="text-sm text-gray-600 mb-1">
                                <span class="font-semibold text-gray-800">Data:</span> {{ \Carbon\Carbon::parse($pedido->data)->format('d/m/Y') }}
                            </p>
                            <p class="text-sm text-gray-600 mb-1">
                                <span class="font-semibold text-gray-800">Status:</span> 
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $pedido->status == 'aberto' ? 'bg-blue-100 text-blue-800' : ($pedido->status == 'processando' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
